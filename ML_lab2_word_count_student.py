...
pluralRDD = wordsRDD.map(lambda word : word + 's')
...
wordCountsGrouped = wordsGrouped.map(lambda (word, count) : (word, sum(count)))
...
def wordCount(wordListRDD):
    ...
    """
#    <FILL IN>
    return wordListRDD.map(lambda s : (s,1)).reduceByKey(lambda a,b : a+b)
...
def removePunctuation(text):
    ...
    """
#    <FILL IN>
    return re.sub('[^0-9A-Za-z\s]', '', text).lower().strip()
...
shakeWordsRDD = shakespeareWordsRDD.filter(lambda line: len(line.strip())>0)
