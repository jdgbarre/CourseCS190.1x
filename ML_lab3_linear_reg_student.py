...
def parsePoint(line):
    ...
    return LabeledPoint(splitString[0], splitString[1:]) 
...
def calcRMSE(labelsAndPreds):
    ...
    RMSE = np.sqrt(MSE / labelsAndPreds.count())
...
#        gradient = <FILL IN>
        ...
        gradient = trainData.map(lambda x: gradientSummand(w, x)).reduce(lambda x1, x2: x1+x2)
...
def twoWayInteractions(lp):
    ...
    """
#    <FILL IN>
    newFeatures = np.array([a*b for (a,b) in itertools.product(lp.features,repeat=2)])
