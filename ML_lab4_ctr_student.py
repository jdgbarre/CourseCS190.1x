...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    """
#    <FILL IN>
    value = {OHEDict[item]:1.0  for item in rawFeats }
    return SparseVector(numOHEFeats, value)
...
#    <FILL IN>
    label = point.split(',')[0]
    rawFeats = parsePoint(point)  # split + return (featureID, value)
    features = oneHotEncoding(rawFeats, OHEDict, numOHEFeats)  # return SparseVector
...
#    <FILL IN>
    ...
    sortedList = sorted(sizeList)
    value = [1 for f in rawFeats if f in OHEDict ]
    
    return SparseVector(numOHEFeats, sortedList, value)
...
def parseHashPoint(point, numBuckets):
    ...
    """
#    <FILL IN>
    hashFeatures = hashFunction(numBuckets, parsePoint(point), True)
    sparcedFeatures = SparseVector(numBuckets, hashFeatures)
    label = point.split(',')[0]
    return LabeledPoint(label, sparcedFeatures)
